...
class NumberArrayTest {
    ...
    void testMinOneNumber() {
        assertEquals(BigInteger.valueOf(15), this.oneNumberArray.min());
    }

    @Test
    void testMaxOneNumber() {
        assertEquals(BigInteger.valueOf(15), this.oneNumberArray.max());
    }

    @Test
    void testSumOneNumber() {
        assertEquals(BigInteger.valueOf(15), this.oneNumberArray.sum());
    }

    @Test
    void testMultOneNumber() {
        assertEquals(BigInteger.valueOf(15), this.oneNumberArray.mult());
    }

    @Test
    void testMinMillionRandomNumbers() {
        BigInteger expectedMin = BigInteger.valueOf(Collections.min(millionRandomNumbers));
        assertEquals(expectedMin, millionNumberArray.min());
    }

    @Test
    void testMaxMillionRandomNumbers() {
        BigInteger expectedMax = BigInteger.valueOf(Collections.max(millionRandomNumbers));
        assertEquals(expectedMax, millionNumberArray.max());
    }
    ...
    void testSumMillionRandomNumbers() {
        ...
        assertEquals(expectedSum, millionNumberArray.sum());
    }
    ...
    void testMultBigIntegers() {
        ...
        assertEquals(expectedProduct, bigIntArray.mult());
    }
    ...
    @Test
    void testCreateFromFileMillionRandomIntegers() throws IOException {
        File tempFile = File.createTempFile("numbers", ".txt");
        BufferedWriter writer = new BufferedWriter(new FileWriter(tempFile));
        writer.write(this.millionRandomNumbers.toString().replaceAll("[\\[\\],]", ""));
        writer.close();

        NumberArray numberArray = NumberArray.createFromFile(tempFile.getPath());

        assertEquals(this.millionNumberArray, numberArray);
        tempFile.delete();
    }
